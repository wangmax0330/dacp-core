	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
		...
		if (!uri.matches(RESOURCE_PATTERN)) {
			...
			if (!"all".equalsIgnoreCase(EXCEPT_PATH_REGEX) && !uri.matches(EXCEPT_PATH_REGEX)) {
				...
				user = sessionService.getCurrentUser(req, SystemUserDomain.class);

				if (user == null) {// 跳转到登录页面
					String loginPage = Configuration.getInstance().getProperty(
							"com.pikia.component.web.DefaultFilter.loginPage");
